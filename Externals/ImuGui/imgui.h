...
namespace ImGui
{
    ...
    // - In this header file we use the "label"/"m_name" terminology to denote a string that will be displayed and used as an ID,
    ...
    // - Those are merely shortcut to calling Text() with a format string. Output single value in "m_name: value" format (tip: freely declare more in your code to handle your types. you can add functions to the ImGui namespace)
    ...
    IMGUI_API void          SetTabItemClosed(const char* tab_or_docked_window_label);           // notify TabBar or Docking system of a closed tab/window ahead (useful to reduce visual flicker on reorderable tab bars). For tab-bar: call after BeginTabBar() and before Tab submissions. Otherwise call with a window m_name.
    ...
    IMGUI_API void          CaptureKeyboardFromApp(bool want_capture_keyboard_value = true);    // attention: misleading m_name! manually override io.WantCaptureKeyboard flag next frame (said flag is entirely left for your application to handle). e.g. force capture keyboard when your widget is being hovered. This is equivalent to setting "io.WantCaptureKeyboard = want_capture_keyboard_value"; after the next NewFrame() call.
    IMGUI_API void          CaptureMouseFromApp(bool want_capture_mouse_value = true);          // attention: misleading m_name! manually override io.WantCaptureMouse flag next frame (said flag is entirely left for your application to handle). This is equivalent to setting "io.WantCaptureMouse = want_capture_mouse_value;" after the next NewFrame() call.
...
enum ImGuiStyleVar_
{
    // Enum m_name --------------------- // Member in ImGuiStyle structure (see ImGuiStyle for descriptions)
...
struct ImGuiIO
{
    ...
    // Optional: Platform/Renderer back-end m_name (informational only! will be displayed in About Window) + User data for back-end/wrappers to store their own stuff.
...
// Callbacks (follow a flag m_name and see comments in ImGuiInputTextFlags_ declarations for more details)
...
struct ImDrawList
{
    ...
    const char*             _OwnerName;         // Pointer to owner window's m_name for debugging
